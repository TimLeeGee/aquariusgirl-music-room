...
- 0.1.28 最新 hotfix 是 `Kill Metadata Save Loop`：播放統計、duration、歌曲資訊 / 封面保存走單曲 `put` / `patch`；歌曲資訊面板有「儲存到播放器」與「套用到原始檔」兩條路徑；播放佇列會跟目前歌曲清單排序由上到下播放；新增 dev guard 可警示重複 stored metadata 回灌、播放中非預期原檔 metadata 重讀、同 track source 變動造成的 `audio.load()`。
