...
- Aquariusgirl Music Room 是 React + TypeScript + Vite + Electron 的本地音樂播放器，目前版本是 `0.1.43`。
- 0.1.43 最新 hotfix 是 `Big Cover Readback Crash / Save Feedback`：使用者以 macOS DMG 實測回報，MP3 用 320KB `cover 01.jpeg` 換封面成功，改用 4.3MB `Cover 01.jpg` 保存後卡住、「重新讀取音樂標籤」一直失敗；Finder 已見新封面、播放器仍舊圖。根因與副檔名無關：taglib-wasm 預設 partial read（前 1MB + 尾 128KB）在寫入 4.3MB 封面後把約 4.3MB 的 ID3v2 標籤截斷，packaged Emscripten TagLib 解析截斷 buffer 直接 WASM `RuntimeError: unreachable`（`isValid()` 仍 true、崩潰在 properties / getPictures / dispose），不是 `InvalidFormatError`，0.1.41 的 retry 接不到 → 寫回成功但 readback / reload 永遠失敗。次因：`MessageToast` z-[60] 被面板 overlay z-[80] 蓋住，失敗訊息看不到才像「卡住」。修法（4 檔案約 40 行、零新套件、不動寫回與 readback hash 路徑）：`readSongInfoFromOriginalFile(sourcePath, { partialRead })` 預設完整讀取（單檔 user-initiated 動作：寫前預讀 / readback / 重新讀取），partial 路徑任何錯誤都 fallback 一次 `partial:false`，`readPicturesSafely` 遇 `WebAssembly.RuntimeError` rethrow 交給 fallback；`electron/selectedFile.ts` 掃描明確走 `partialRead: true` 維持上萬首載入效能，大封面檔自癒；`MessageToast` 升 `z-[90]`；`SongInfoPanel` 保存中顯示「套用中…」。Linux 沙盒以打包版 wasm 設定重現舊崩潰並驗證修後 320KB / 4.3MB 寫入＋讀回 hash 通過；全部 `check:*`、`tsc --noEmit`、`electron:compile` PASS；installer 由 `打包發行.command` 產出，SHA-256 見 `docs/releases/0.1.43-checksums.md`；未 push GitHub。
