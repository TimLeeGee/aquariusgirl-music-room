...
- 補做 packaged macOS 滑鼠 smoke：0.1.26 DMG 可開啟，`01. Plazma.flac` 可播放，切到 `02. BOW AND ARROW.flac` 再切回 `01. Plazma.flac` 後進度可前進，暫停可切回播放按鈕。
- 限制：未做封面寫回滑鼠驗收，因為目前自動載入的是使用者原始 Music 路徑，不是暫存複本；Windows 真機仍待驗收。
...
尚未完成：封面寫回滑鼠驗收、Windows 真機安裝、播放中更換封面後切歌再切回不卡、cover02 -> cover01 第一次重開不回跳、選擇約 4 GB / 20+ 首音樂資料夾、重開後恢復最後資料夾、歌曲資訊編輯、改封面後重開播放清單不掉歌、AI 建歌單等待狀態實機操作、Apple Developer ID / notarization、Windows code signing。
...
- Packaged macOS mouse smoke passed for opening the 0.1.26 DMG app, playing `01. Plazma.flac`, switching to `02. BOW AND ARROW.flac`, switching back, observing progress advance, and pausing back to the play button. Cover-writeback mouse QA remains open because the loaded source was the user's original Music folder, not a temp copy.
