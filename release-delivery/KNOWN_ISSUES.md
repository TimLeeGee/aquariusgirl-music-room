...
- Windows 真機尚未安裝 `Aquariusgirl Music Room Setup 0.1.17.exe` 驗證 fresh install、啟動、選擇音樂資料夾、AI 聊天與 AI 建歌單。
...
- 0.1.17 內建模型與 llama.cpp runtime，因此 installer 體積明顯大於純播放器版本。
...
## 0.1.17 Manual QA Still Needed

- `Aquariusgirl Music Room Setup 0.1.17.exe` has not been installed on a real Windows machine for fresh install, launch, folder selection, AI chat, and AI playlist creation.
...
- 0.1.17 bundles the model and llama.cpp runtime, so installers are much larger than player-only builds.
