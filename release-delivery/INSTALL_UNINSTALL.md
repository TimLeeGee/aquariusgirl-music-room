...
0.1.48（面板文字全量登錄表）installer 已產出並同步到 `release-delivery/installers/`；安裝／解除安裝步驟與先前版本相同，檔名改為 0.1.48。SHA-256 請看 `docs/releases/0.1.48-checksums.md`。（0.1.45–0.1.47 安裝步驟亦相同，檔名對應各版，SHA-256 見各自 checksums 文件。）

## 0.1.44 安裝狀態

0.1.44（確認窗焦點鎖死與提示位置修正）installer 已產出並同步到 `release-delivery/installers/`；安裝步驟與 0.1.43 相同，檔名改為 0.1.44。SHA-256 請看 `docs/releases/0.1.44-checksums.md`。

## 0.1.43 安裝狀態

0.1.43（大封面讀回崩潰與保存提示修正）installer 已產出並同步到 `release-delivery/installers/`；安裝步驟與 0.1.42 相同，檔名改為 0.1.43。SHA-256 請看 `docs/releases/0.1.43-checksums.md`。
