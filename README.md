...
- Normal playlists and smart playlists.
...
- macOS Apple Silicon DMG and Windows EXE packaging.
...
The desktop app can bundle a local llama.cpp runtime and `qwen3.5-0.8b.gguf`. Users do not need Ollama, Node.js, a terminal, or cloud API access.

The AI only receives a safe metadata summary of the currently loaded music library. It does not upload music files, local paths, artwork blobs, `File` objects, or `ArrayBuffer` data.

Prompt files are open text files:

- `private/prompts/character_prompt.txt`
- `private/prompts/ai_router_prompt.txt`
- `private/prompts/ai_reply_prompt.txt`

The small model only routes intent and polishes short replies. The app code performs local search, random playlist creation, playlist creation, playlist insertion, and safe playlist removal behavior. If model JSON parsing fails, the app falls back to deterministic rules instead of showing raw model output.
...
The player stores metadata, playlists, and settings. It does not persist music file bytes, `File` objects, object URLs, or absolute user paths in exported backups.

The web version may require users to select files or folders again after refresh because browsers do not allow permanent arbitrary disk access. Electron can preserve metadata, but it does not scan the disk without explicit user selection.

## Playlists

The built-in system playlists are:

- All Songs
- Liked Songs

Users can create normal playlists and smart playlists. Normal playlists store ordered track IDs. Smart playlists store rules and filter the current metadata dynamically.

## Mini Player

The Mini player uses the same playback state and the same `BrowserWindow`. It does not create a second audio element. Switching modes preserves the current track, playback position, volume, and play state.
...
Unsigned development builds may trigger macOS Gatekeeper or Windows SmartScreen warnings. Official public releases should use Apple Developer ID notarization and Windows code signing.

Do not commit certificates, private keys, local music files, generated installers, build output, `node_modules`, or the large GGUF model file.
